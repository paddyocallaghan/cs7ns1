# Source: https://github.com/tedski999/tcdicn/tree/main
# Justification: We implemented this code found at the above source
# for the basic networking portion of this assignment
# Changes were made to facilite our implementation, and are
# commented to represent this
...
async def _send_get_msg(
    peer: _PeerId, ttp: float, eol: float, tag: _Tag, last_time: float, id: _ClientId
):
...
# Server edited to implement SSL
...
class Server:
    ...
    def _on_udp_data(self, msg_bytes: bytes, peer: _PeerId):
        ...
        try:
            ...
            logging.warning(f"Received bad JSON from {peer}: {e}; ignoring.")
            return

            # 检查消息是否包含所有必需的键
        required_keys = ["version", "type", "eol", "clients"]
        if not all(key in msg for key in required_keys):
            # logging.warning(f"Received incomplete datagram from {peer}; ignoring.")
            return
        # Parse advertisement
        msg = json.loads(msg_bytes)
...
# Client edited to add SSL context for encryption - Paddy
