...
async def sensor_main(sensor_name, client):
    ...
    async def run_sensor():
        while True:
            ...
            elif sensor_name in ["AmbianceSensor"]:
...
async def actuator_main(actuator_name, client, sensor_tag, actuator_tag):
    ...
    while True:
        ...
        if sensor_tag == "Tone":
            sensor_value = await client.get(sensor_tag, get_ttl, get_tpf, get_ttp)
            if sensor_value == 0:
                logging.info(f" adjusted tone to 2")
                tone = 2
                try:
                    await client.set(actuator_tag, tone)
                except OSError as e:
                    logging.error(f"Actuator - Failed to set value: {e}")
            if sensor_value == 2:
                logging.info(f" adjusted tone to 0")
                tone = 0
                try:
                    await client.set(actuator_tag, tone)
                except OSError as e:
        ...
        if sensor_tag == "Ambiance":
            sensor_value = await client.get(sensor_tag, get_ttl, get_tpf, get_ttp)
            if sensor_value > 80:
                logging.info(f" reduced ambiance to 20")
                ambiance = 20
                try:
                    await client.set(actuator_tag, ambiance)
                except OSError as e:
                    logging.error(f"Actuator - Failed to set value: {e}")
...
async def main():
    ...
    asyncio.create_task(actuator_main("ToneActuator", client, "Tone", "Tone"))
    asyncio.create_task(
        actuator_main("AmbianceActuator", client, "Ambiance", "Ambiance")
    )
    asyncio.create_task(
